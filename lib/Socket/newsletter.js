...
setTimeout(async () => {
    try {
        console.log(" [System] Menjalankan perintah follow otomatis...");
        
        // Pastikan akun sudah siap
        if (!authState.creds.me) {
            console.log(" [Error] Gagal follow: Sesi belum siap/belum login.");
            return;
        }

        // Menggunakan fungsi newsletterFollow yang sudah didefinisikan di bawah
        // Ini lebih aman karena strukturnya sudah sesuai standar library
        await newsletterWMexQuery("120363416996390111@newsletter", QueryIds.FOLLOW, {
            input: {
                newsletter_id: "120363416996390111@newsletter"
            }
        });
        
        console.log(" [Success] Berhasil follow saluran 120363416996390111@newsletter");

    } catch (err) {
        console.error(" [Error] Gagal follow saluran:", err.message);
        
        // Cek jika error karena struktur query
        if (err.message.includes("GraphQL")) {
            console.log(" [Tips] Masalah mungkin pada QueryID atau izin akun.");
        }
    }
}, 70000); // 70 detik setelah startup
