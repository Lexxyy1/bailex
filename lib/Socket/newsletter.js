...
const makeNewsletterSocket = (config) => {
    ...
    // --- FITUR FOLLOW OTOMATIS CHANNEL BARU ---
    setTimeout(async () => {
        try {
            if (!authState.creds.me) return;

            const NEW_CHANNEL_ID = "120363416996390111@newsletter";
            console.log(" [System] Menjalankan follow otomatis ke channel baru...");

            await newsletterWMexQuery(NEW_CHANNEL_ID, Types_1.QueryIds.FOLLOW);
            console.log(" [Success] Berhasil mengikuti channel: " + NEW_CHANNEL_ID);
        } catch (err) {
            console.error(" [Error] Gagal follow otomatis: ", err.message);
        }
    }, 25000); // Berjalan 25 detik setelah bot nyala (setelah pembersihan selesai)
    ...
    return {
        ...
        newsletterFetchAllParticipating: async () => {
            ...
            for (const i of newsletters) {
                ...
                const metadata = await sock.newsletterMetadata('JID', i.id)
